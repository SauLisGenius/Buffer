...
namespace PostmanFriend.GameScripts
{
    class MiaoJi
    {
            ...
            try
            {
                ...
                while (_postManPower._clientWebSocket.State == System.Net.WebSockets.WebSocketState.Open && !getData)
                {
                    ...
                    if (message.Contains("BetAndAward")) {
                        MiaoJiStartGameAPI miaoJiStartGameAPI = JsonConvert.DeserializeObject<MiaoJiStartGameAPI>(message.Replace("", ""));
                        string result = miaoJiStartGameAPI.result;
                        MiaoJiStartGameResult2 miaoJiStartGameResult2 = JsonConvert.DeserializeObject<MiaoJiStartGameResult2>(result);
        ...
        public async Task<long> Spin(long bet)
        {
            ...
            while (_postManPower._clientWebSocket.State == System.Net.WebSockets.WebSocketState.Open && !getData)
            {
                if (message.Contains("NoBonusTimes") && message.Contains("Reward"))
                {
                    MiaoJiStartGameAPI miaoJiStartGameAPI = JsonConvert.DeserializeObject<MiaoJiStartGameAPI>(message.Replace("", ""));
                    string result = miaoJiStartGameAPI.result;
                    MiaoJiStartGameResult miaoJiStartGameResult = JsonConvert.DeserializeObject<MiaoJiStartGameResult>(result);

                    //一般
                    score = miaoJiStartGameResult.AwardStateObject.Reward;

                    //Bonus
                    if (miaoJiStartGameResult.AwardStateObject.Bonus != null)
                    {
                        long bonusScore = 0;
                        foreach (var item in miaoJiStartGameResult.AwardStateObject.Bonus)
                        {
                    ...
                    //外送得分
                    if (miaoJiStartGameResult.Grab7Event.Current == 0 && miaoJiStartGameResult.AwardStateObject.Bonus != null)
                    {
                        long extra = miaoJiStartGameResult.Grab7Event.Bet / 3 * miaoJiStartGameResult.Grab7Event.Reward;//一枚硬幣的價值*200 = 外送得分
                        score += extra;
                    }
                    ...
                    if (score == 0)
                    {
                        if (miaoJiStartGameResult.AwardStateObject.Award == 4)
                        {
        ...
        /// </summary>
        /// <returns></returns>
        string TranQuote(string content)
        {
            string newContent = "";
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<MiaoJiStartGameResult2> GetMachineInfoAward()
        {
            MiaoJiStartGameResult2 miaoJiStartGameResult2 = new MiaoJiStartGameResult2();
            try
            {
                //GetMachineInfo
                object arguments = new object[] { };
                ...
                while (_postManPower._clientWebSocket.State == System.Net.WebSockets.WebSocketState.Open && !getData)
                {
                    ...
                    if (message.Contains("BetAndAward"))
                    {
                        MiaoJiStartGameAPI miaoJiStartGameAPI = JsonConvert.DeserializeObject<MiaoJiStartGameAPI>(message.Replace("", ""));
                        string result = miaoJiStartGameAPI.result;
                        miaoJiStartGameResult2 = JsonConvert.DeserializeObject<MiaoJiStartGameResult2>(result);
                        //Award = miaoJiStartGameResult2.BetAndAward[0].Award;//[0] = 下注額300，[1] = 下注額600，[2] = 下注額900
                        //Console.WriteLine("Award " + Award);
