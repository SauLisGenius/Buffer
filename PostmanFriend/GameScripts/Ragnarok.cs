...
using PostmanFriend.Protocols.Ragnarok;
    ...
    class Ragnarok
    {
        ...
        public async Task<long> Spin(string uri, string path, object data, Dictionary<string, string> header = null)
        {
            ...
            try
            {
                ...
                if (result.IndexOf("\"totalScore\"") != -1)
                {
                    RagnarokStartGameAPI ragnarokStartGameAPI = JsonConvert.DeserializeObject<RagnarokStartGameAPI>(result);
                    score = ragnarokStartGameAPI.svData.str.totalScore;
                }
            }
            catch (Exception ex)
            {
