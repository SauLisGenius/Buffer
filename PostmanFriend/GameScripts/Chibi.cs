...
using PostmanFriend.Protocols.Chibi;
    ...
    class Chibi
    {
        ...
        public async Task<long> Spin(string uri, string path, object data, Dictionary<string, string> header = null)
        {
            ...
            try
            {
                ...
                if (result.IndexOf("\"iWinPoint\"") != -1)
                {
                    ChibiStartGameAPI chibiStartGameAPI = JsonConvert.DeserializeObject<ChibiStartGameAPI>(result);

                    long sum = 0;
                    foreach (var item in chibiStartGameAPI.svData.lsRound)
                    {
                        sum += item.iWinPoint;
                    }
