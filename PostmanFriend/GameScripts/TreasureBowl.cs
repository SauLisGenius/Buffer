...
namespace PostmanFriend.GameScripts
{
    class TreasureBowl
    {
        ...
        public async Task<long> GetMachineInfo()
        {
            ...
            try
            {
                ...
                while (_postManPower._clientWebSocket.State == System.Net.WebSockets.WebSocketState.Open && !getData)
                {
                    ...
                    {
                        TreasureBowlStartGameAPI treasureBowlStartGameAPI = JsonConvert.DeserializeObject<TreasureBowlStartGameAPI>(message.Replace("", ""));
                        string result = treasureBowlStartGameAPI.result;
                        TreasureBowlStartGameResult2 treasureBowlStartGameResult2 = JsonConvert.DeserializeObject<TreasureBowlStartGameResult2>(result);
                        Award = treasureBowlStartGameResult2.BetAndAward[0].Award;//[0] = 下注額300，[1] = 下注額600，[2] = 下注額900
        ...
        public async Task<long> Spin(long bet)
        {
            ...
            while (_postManPower._clientWebSocket.State == System.Net.WebSockets.WebSocketState.Open && !getData)
            {
                if (message.Contains("NoBonusTimes") && message.Contains("Reward"))
                {
                    TreasureBowlStartGameAPI treasureBowlStartGameAPI = JsonConvert.DeserializeObject<TreasureBowlStartGameAPI>(message.Replace("", ""));
                    string result = treasureBowlStartGameAPI.result;
                    TreasureBowlStartGameResult treasureBowlStartGameResult = JsonConvert.DeserializeObject<TreasureBowlStartGameResult>(result);

                    //一般
                    score = treasureBowlStartGameResult.AwardStateObject.Reward;

                    //Bonus
                    if (treasureBowlStartGameResult.AwardStateObject.Bonus != null)
                    {
                        long bonusScore = 0;
                        foreach (var item in treasureBowlStartGameResult.AwardStateObject.Bonus)
                        {
                    ...
                    //外送得分
                    if (treasureBowlStartGameResult.Grab7Event.Current == 0 && treasureBowlStartGameResult.AwardStateObject.Bonus != null)
                    {
                        long extra = treasureBowlStartGameResult.Grab7Event.Bet / 3 * treasureBowlStartGameResult.Grab7Event.Reward;//一枚硬幣的價值*200 = 外送得分
                        score += extra;
                    }
                    ...
                    if (score == 0)
                    {
                        if (treasureBowlStartGameResult.AwardStateObject.Award == 4)
                        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<TreasureBowlStartGameResult2> GetMachineInfoAward()
        {
            TreasureBowlStartGameResult2 treasureBowlStartGameResult2 = new TreasureBowlStartGameResult2();
            try
            {
                //GetMachineInfo
                object arguments = new object[] { };
                ...
                while (_postManPower._clientWebSocket.State == System.Net.WebSockets.WebSocketState.Open && !getData)
                {
                    ...
                    if (message.Contains("BetAndAward"))
                    {
                        TreasureBowlStartGameAPI treasureBowlStartGameAPI = JsonConvert.DeserializeObject<TreasureBowlStartGameAPI>(message.Replace("", ""));
                        string result = treasureBowlStartGameAPI.result;
                        treasureBowlStartGameResult2 = JsonConvert.DeserializeObject<TreasureBowlStartGameResult2>(result);
            ...
            return treasureBowlStartGameResult2;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<NoBonusTimeSVData> GetMachineInfo120(string uri, string path, Dictionary<string, string> header = null)
        {
            NoBonusTimeSVData noBonusTimeSVData = null;

            try
            {
                string result = await _postMan.HttpGetAsync(uri, path, header);

                if (result.IndexOf("\"betAndNoBonusTimes\"") != -1)
                {
                    noBonusTimeSVData = JsonConvert.DeserializeObject<NoBonusTimeAPI>(result).svData;
                }
            }
            catch (Exception ex)
            {
